<x-splade-component is="button-with-dropdown" dusk="bulk-actions-exports-dropdown" v-bind:close-on-click="true">
    ...
    <div class="min-w-max rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 dark:bg-gray-600 dark:text-gray-100">
        <div class="flex flex-col">
            ...
            @foreach($table->getBulkActions() as $bulkAction)
                <button
                    ...
                    class="text-left w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-gray-900 font-normal dark:text-gray-100 dark:bg-gray-600 dark:hover:bg-gray-500 dark:hover:text-gray-300"
            ...
            @if($table->hasExports() && $table->hasBulkActions())
                <div v-if="table.hasSelectedItems" class="border-t w-full dark:border-gray-700"></div>
            @endif

            @if($table->hasExports())
                <h3 class="text-xs uppercase tracking-wide bg-gray-100 px-4 py-2 border-b dark:bg-gray-600 dark:text-gray-100">
                    {{ __('Export results') }}
                </h3>
            @endif
            ...
            @foreach($table->getExports() as $export)
                <a
                    download
                    class="text-left w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-gray-900 font-normal dark:text-gray-100 dark:bg-gray-600"
                    href="{{ $export->getUrl() }}"
